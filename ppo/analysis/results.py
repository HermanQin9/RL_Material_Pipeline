...
def find_latest_checkpoint(models_dir: Path, pattern: str = "ppo_agent*.pth") -> Path:
 ...
 if not candidates:
 raise FileNotFoundError(f" {models_dir} PPO (pattern={pattern})")
 return candidates[0]
...
def load_training_data(checkpoint_path: Path) -> tuple[list[float], list[float]]:
 ...
 if not rewards:
 raise ValueError(f" {checkpoint_path} episode_rewards")
...
def create_visualizations(
 ...
 ax1.plot(moving_eps, moving_avg, color="#d62728", linewidth=2, label=f"{window}")
...
def print_summary(
 ...
 print(f" / Total Episodes: {int(stats['count'])}")
 print(f" / Mean Reward: {stats['mean']:.3f} ± {stats['std']:.3f}")
 print(f" / Best Reward: {stats['max']:.3f}")
 print(f" / Worst Reward: {stats['min']:.3f}")
 print(f"SUCCESS (> -0.95): {success_rate * 100:.1f}% ({int(np.sum(success_flags))}/{len(success_flags)})")
 ...
 trend = " "
 print(f" : {stats['first_half_mean']:.3f}")
 print(f" : {stats['second_half_mean']:.3f}")
 print(f"START / Reward Shift: {improvement:+.3f} ({trend})")

 failures = [idx + 1 for idx, flag in enumerate(success_flags) if flag == 0]
 if failures:
 print(f"WARNING (reward -0.95): {failures}")
 else:
 print("SUCCESS / No failing episodes detected")
